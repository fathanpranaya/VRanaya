...
int app_on_http_req(app_t *self, libwebsocket *socket, char *request) {
	...
	else if( strcmp(request, "/socket.io.js") == 0 ) {
		libwebsockets_serve_http_file(self->server->context, socket, "client/socket.io.js", "text/javascript; charset=utf-8", NULL);
		return true;
	}
	else if( strcmp(request, "/gl-matrix.js") == 0 ) {
		libwebsockets_serve_http_file(self->server->context, socket, "client/gl-matrix.js", "text/javascript; charset=utf-8", NULL);
		return true;
	}
	return false;
}
